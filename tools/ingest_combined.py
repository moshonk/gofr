...
Strategy:
  ...
  2. Emit one GofrJurisdiction Location per unique node, with partOf chain.
     Mapping to gofr-jurisdiction-type (constrained ValueSet of country/region/
     district/county): province->region, district->district, od->district,
  ...
  5. POST one transaction Bundle per ~50 facilities to HAPI DEFAULT.
"""
import csv
import json
import os
...
def build_jurisdiction(level, code, name_en, name_kh, parent_ref, jur_type_code):
    loc = {
        ...
        "meta": {
            ...
            "profile": [
                ...
                "http://gofr.org/fhir/StructureDefinition/gofr-jurisdiction",
            ],
        },
        "status": "active",
...
def collect_hierarchy(rows):
    ...
    nodes = {
        "province": OrderedDict(),  # code -> (name_en, name_kh, parent)
    ...
    for r in rows:
        ...
        if pro:
            nodes["province"].setdefault(pro, (r["PRO_NAME_EN"], r["PRO_NAME_KH"], None))
        if dis and pro:
            nodes["district"].setdefault(dis, (r["DIS_NAME_EN"], r["DIS_NAME_KH"], pro))
        if od and dis:
            nodes["od"].setdefault(od, (r["OD_NAME_EN"], r["OD_NAME_KH"], dis))
        if com and (od or dis):
            nodes["commune"].setdefault(
                com,
                (r["COM__NAME_EN"], r["COM_NAME_KH"], od or dis),
            )
        if vil and com:
            nodes["village"].setdefault(vil, (r["VIL__NAME_EN"], r["VIL_NAME_KH"], com))
    return nodes
...
PARENT_LEVEL = {
    ...
    "district": "province",
    "od": "district",
    "commune": "od",  # may also be district
    "village": "commune",
}
...
def main():
    ...
    # Build jurisdiction Locations in dependency order
    jurisdiction_resources = []
    for level in ("province", "district", "od", "commune", "village"):
        for code, (name_en, name_kh, parent_code) in nodes[level].items():
            parent_level = PARENT_LEVEL[level]
            parent_ref = None
            if parent_code and parent_level:
                parent_ref = f"Location/{jur_id(parent_level, parent_code)}"
            jurisdiction_resources.append(build_jurisdiction(
                level=level,
                code=code,
                name_en=name_en or "",
                name_kh=name_kh or "",
    ...
    print(f"Built: {len(jurisdiction_resources)} jurisdictions, "
    ...
    # Phase 2: jurisdictions in dependency order, batched
    print("\n--- Phase 2: Jurisdictions ---")
    for level in ("province", "district", "od", "commune", "village"):
        level_resources = [j for j in jurisdiction_resources if j["id"].startswith(f"{level}-")]
        for i in range(0, len(level_resources), BATCH_SIZE):
            ...
            errors = [e for e in bundle_resp.get("entry", [])
                      if not str(e.get("response", {}).get("status", "")).startswith(("200", "201"))]
            print(f"  {level} batch {i//BATCH_SIZE+1}: {len(batch)} resources -> HTTP {status}, {len(errors)} errors")
            if errors:
